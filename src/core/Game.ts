...
const OFFICE_PLAYER_RUN_SPEED = GAME_CONFIG.player.sprintSpeed * OFFICE_PLAYER_SPRINT_SPEED_MULTIPLIER;
const OFFICE_GAME_MODE_CHASE_MATCH_SPEED = OFFICE_PLAYER_RUN_SPEED;
const OFFICE_FOXY_RUSH_SPEED = OFFICE_PLAYER_RUN_SPEED;
...
const MICROPHONE_JUMPSCARE_RECORDING_ID: string | null = null;
...
export class Game {
  ...
  private officeGoldenBoriLastTranscript = '';
  private officeGoldenBoriLastTranscriptTimer = 0;
  private readonly officeGoldenBoriRecentReplies: string[] = [];
  ...
  private officeEmployeeElevatorAtBasement = false;
  ...
  private readonly resize = (): void => {
          ...
          : this.chapterFiveActive
            ...
            : this.chapterSixActive
              ...
              : this.chapterSevenActive
                ? 0.9
                : this.chapterEightActive || this.chapterNineActive
                  ? 0.9
                : 1;
    ...
    this.renderer.setPixelRatio(
      this.doomModeActive || this.officeChapterActive || this.chapterFourActive
        || this.chapterFiveActive || this.chapterSixActive || this.chapterSevenActive || this.chapterEightActive || this.chapterNineActive
  ...
  private selectOfficeGoldenBoriReply(options: readonly string[]): string {
    const available = options.filter((line) => !this.officeGoldenBoriRecentReplies.includes(line));
    const choices = available.length > 0 ? available : options;
    const line = choices[this.officeGoldenBoriConversationLineIndex % choices.length] ?? options[0] ?? 'I heard you.';
    this.officeGoldenBoriConversationLineIndex += 1;
    this.officeGoldenBoriRecentReplies.push(line);
    while (this.officeGoldenBoriRecentReplies.length > 5) {
      this.officeGoldenBoriRecentReplies.shift();
    }
    return line;
  }
  ...
  private getOfficeGoldenBoriConversationReply(normalized: string, insulted: boolean): string {
    ...
    if (/\b(hello|hi|hey|yo|sup|what's up|whats up)\b/.test(normalized)) {
      const greetingReplies = [
        ...
        'Hi. Keep talking and I will keep listening.',
      ];
      return this.selectOfficeGoldenBoriReply(greetingReplies);
    }
    ...
    if (/\b(how are you|how have you been|how you doing|how are ya|how do you feel)\b/.test(normalized)) {
      const moodReplies = [
        ...
        'Better now. Conversations are more interesting than rehearsal lines.',
      ];
      return this.selectOfficeGoldenBoriReply(moodReplies);
    }

    if (/\b(who are you|your name|what are you)\b/.test(normalized)) {
      return this.selectOfficeGoldenBoriReply([
        "I'm Golden Bori. The show gets louder when I wake up.",
        'I am the golden one in the game room, and I hear more than people think.',
        'Golden Bori. Bigger shine, heavier steps, louder thoughts.',
      ]);
    }

    if (/\b(are you alive|can you hear|you hear me|listen)\b/.test(normalized)) {
      return this.selectOfficeGoldenBoriReply([
        'I hear every word in this room.',
        'I can hear you from farther than you think.',
        'The microphone wakes up my ears before my feet move.',
      ]);
    }

    if (/\b(pizza|pizzeria|restaurant|building)\b/.test(normalized)) {
      return this.selectOfficeGoldenBoriReply([
        "This pizzeria is old, loud, and full of places where footsteps echo.",
        'The building remembers parties, alarms, and people running through halls.',
        'Every room in this pizzeria has a different sound when someone is hiding.',
      ]);
    }

    if (/\b(basement|control room|elevator|keycard|rubble)\b/.test(normalized)) {
      return this.selectOfficeGoldenBoriReply([
        'The basement keeps its secrets under the floor. The control room knows more than the stage does.',
        'Downstairs is where old machines keep deciding what still works.',
        'The control room is not friendly. It is useful, but not friendly.',
      ]);
    }

    if (/\b(foxy|pirate)\b/.test(normalized)) {
      return this.selectOfficeGoldenBoriReply([
        'Foxy likes a brave audience. He also likes when people run.',
        'Foxy acts like a performer, but he listens like a hunter.',
        'If Foxy is quiet, that does not mean he is gone.',
      ]);
    }

    if (/\b(quacky|duck)\b/.test(normalized)) {
      return this.selectOfficeGoldenBoriReply([
        'Quacky sings bright songs, but her smile is not as soft as it looks.',
        'Quacky hears rhythm in footsteps. That makes her hard to fool.',
        'Her beak is made for songs, but it can open for worse things.',
      ]);
    }

    if (/\b(fluffle|bunny|rabbit)\b/.test(normalized)) {
      return this.selectOfficeGoldenBoriReply([
        'Fluffles moves quick when the room gets quiet.',
        'Fluffles waits for a mistake, then turns it into a chase.',
        'Do not trust how still Fluffles looks.',
      ]);
    }

    if (/\b(animatronic|animatronics|robots|robot)\b/.test(normalized)) {
      return this.selectOfficeGoldenBoriReply([
        'Animatronics remember sounds, routes, and the people who get too comfortable.',
        'We are built from routines, but routines can learn bad habits.',
        'An animatronic does not need to hurry if it knows where you are going.',
      ]);
    }

    if (/\b(camera|cameras|picture|photo|printer)\b/.test(normalized)) {
      return this.selectOfficeGoldenBoriReply([
        'Cameras catch little moments. Sometimes one picture is enough to unlock the next problem.',
        'A photo only proves what was there for one second.',
        'The printer likes proof. The cameras like timing.',
      ]);
    }

    if (/\b(night|dark|darkness|lights)\b/.test(normalized)) {
      return this.selectOfficeGoldenBoriReply([
        'Night makes the halls honest. You hear what is moving before you see it.',
        'In the dark, the quiet parts of the building get louder.',
        'Lights help you see. Darkness helps me listen.',
      ]);
    }

    if (/\b(day|daytime|morning)\b/.test(normalized)) {
      return this.selectOfficeGoldenBoriReply([
        'Daytime is quieter, but I still listen.',
        'Day does not turn me off. It only makes me patient.',
        'Morning makes the place look safer than it is.',
      ]);
    }

    if (/\b(game|games|arcade|play|stage|room)\b/.test(normalized)) {
      return this.selectOfficeGoldenBoriReply([
        'The games are ready. Keep moving and I might keep watching.',
        'The game room is better when someone is brave enough to stay in it.',
        'Stages are for shows. Game rooms are for choices.',
      ]);
    }

    if (/\b(scared|scary|creepy|afraid|danger)\b/.test(normalized)) {
      return this.selectOfficeGoldenBoriReply([
        'Good. A little fear keeps the performance sharp.',
        'Being scared means you are paying attention.',
        'Fear is useful here. It tells you when to move.',
      ]);
    }

    if (/\b(friend|nice|cool|awesome|good|great)\b/.test(normalized)) {
      return this.selectOfficeGoldenBoriReply([
        'That is better. Talk nice and maybe I stay calm.',
        'Nice words sound strange in this place, but I can accept them.',
        'Careful. If you call me cool, I might believe you.',
      ]);
    }
    ...
    if (/\b(tell me about|talk about|what about|think about|do you like|do you know about)\b/.test(normalized)) {
      const topicReplies = [
        ...
        'I have thoughts about it, but I want to hear what you think first.',
        'That is something worth talking about. Stay nearby and keep going.',
      ];
      return this.selectOfficeGoldenBoriReply(topicReplies);
    }
    ...
    if (/\b(why|what|when|where|how)\b/.test(normalized)) {
      const questionReplies = [
        ...
        'I can answer, but I might not answer the same way twice.',
        'That is a real question. I am thinking about it.',
      ];
      return this.selectOfficeGoldenBoriReply(questionReplies);
    }
    ...
    return this.selectOfficeGoldenBoriReply(replies);
  }
  ...
  private maybeHandleOfficeGoldenBoriConversation(normalized: string, insulted: boolean): void {
    ...
    if (normalized === this.officeGoldenBoriLastTranscript && this.officeGoldenBoriLastTranscriptTimer > 0) {
      return;
    }
    if (!addressedGoldenBori && distanceToGoldenBori > 26) {
      return;
    }
    if (!addressedGoldenBori && !conversationalSpeech && distanceToGoldenBori > 16) {
      return;
    }
    if (distanceToGoldenBori > 52) {
      return;
    }

    this.officeGoldenBoriLastTranscript = normalized;
    this.officeGoldenBoriLastTranscriptTimer = 4.5;
    const line = this.getOfficeGoldenBoriConversationReply(normalized, insulted);
    this.officeGoldenBoriConversationCooldown = MathUtils.clamp(4.2 + line.length * 0.055, 5.2, 9.2);
    this.officeBoriAiRehearsalTimer = Math.max(this.officeBoriAiRehearsalTimer, this.officeGoldenBoriConversationCooldown + 14);
  ...
  private readonly update = (deltaSeconds: number): void => {
    ...
    } else if (this.chapterTwoActive || this.officeChapterActive || this.chapterFourActive || this.chapterFiveActive || this.chapterSixActive || this.chapterSevenActive || this.chapterEightActive || this.chapterNineActive) {
    ...
    if (!this.chapterTwoActive && !this.officeChapterActive && !this.chapterFourActive && !this.chapterFiveActive && !this.chapterSixActive && !this.chapterSevenActive && !this.chapterEightActive && !this.chapterNineActive && !this.zombieModeActive && !this.doomModeActive) {
    ...
    if (this.zombieModeActive) {
      ...
      this.chapterNine.update(deltaSeconds, this.player.getPosition());
      this.handleChapterNineJumpscareEvents(deltaSeconds);
    } else if (!this.chapterTwoActive && !this.officeChapterActive && !this.chapterFourActive && !this.chapterFiveActive && !this.chapterSixActive && !this.chapterSevenActive && !this.chapterEightActive && !this.chapterNineActive) {
  ...
  private createOfficePickaxeModel(includeHand = false): Group {
    ...
    const handle = new Mesh(new CylinderGeometry(0.024, 0.034, 0.74, 12), handleMaterial);
    handle.rotation.z = -0.08;
    handle.position.y = -0.08;
    const headRoot = new Group();
    headRoot.position.y = 0.28;
    const collar = new Mesh(new CylinderGeometry(0.055, 0.062, 0.13, 14), metalMaterial);
    collar.rotation.z = Math.PI / 2;
    const makeArm = (side: -1 | 1): void => {
      const inner = new Mesh(new CylinderGeometry(0.026, 0.03, 0.24, 10), metalMaterial);
      inner.position.set(side * 0.11, 0.035, 0);
      inner.rotation.z = side * 1.08;
      const outer = new Mesh(new CylinderGeometry(0.022, 0.026, 0.24, 10), metalMaterial);
      outer.position.set(side * 0.28, 0.075, 0);
      outer.rotation.z = side * 1.34;
      const tip = new Mesh(new ConeGeometry(0.045, 0.17, 10), metalMaterial);
      tip.position.set(side * 0.44, 0.11, 0);
      tip.rotation.z = side > 0 ? -Math.PI / 2 : Math.PI / 2;
      headRoot.add(inner, outer, tip);
    };
    makeArm(-1);
    makeArm(1);
    headRoot.add(collar);
  ...
  private resetOfficePrizeInventory(): void {
    ...
    this.officeGoldenBoriLastTranscript = '';
    this.officeGoldenBoriLastTranscriptTimer = 0;
    this.officeGoldenBoriRecentReplies.length = 0;
    this.clearOfficeHeldPrizeItem();
  }
  ...
  private getChapterLabel(chapter: HudChapterId): string {
    switch (chapter) {
      ...
      case 'chapter-8':
        return 'Chapter 8: The Woods';
      case 'chapter-9':
        return "Chapter 9: Freddy's Pizza Complex";
      case 'zombie-fps':
        return 'Zombie FPS';
      case 'doom-fps':
        return 'Doom Run';
  ...
  private updateAtmosphere(): void {
    ...
    const targetCameraFar = this.chapterFiveActive || this.chapterSixActive || this.chapterSevenActive || this.chapterEightActive || this.chapterNineActive ? 980 : GAME_CONFIG.camera.far;
    ...
    if (this.officeChapterActive) {
      ...
      this.lighting.flashlight.intensity = MathUtils.lerp(
        MathUtils.lerp(
          GAME_CONFIG.flashlight.intensity * 0.74,
  ...
  private getIntroHudState(): { eyebrow: string; title: string; summary: string; buttonText: string } {
    if (this.chapterNineActive) {
      return {
        ...
        title: "Freddy's Pizza Complex",
        summary: 'Record evidence inside a huge abandoned brick pizza complex, survive the locked night, solve puzzles, and escape.',
        buttonText: 'Enter The Complex',
      };
    }
                  ...
                  : this.chapterSevenActive
                    ...
                    : this.chapterEightActive
                      ...
                      : this.chapterNineActive
                        ? "Chapter: Freddy's Pizza Complex"
  ...
  private handleChapterNineJumpscareEvents(deltaSeconds: number): void {
    if (this.chapterNineJumpscare) {
      this.chapterNineJumpscare.elapsed = Math.min(
        this.chapterNineJumpscare.elapsed + deltaSeconds,
        this.chapterNineJumpscare.duration,
      );
      if (this.chapterNineJumpscare.elapsed >= this.chapterNineJumpscare.duration) {
        this.chapterNineJumpscare = null;
      }
    }

    let event = this.chapterNine.consumeJumpscareEvent();
    while (event) {
      this.chapterNineJumpscare = {
        event,
        elapsed: 0,
        duration: 2.4,
      };
      this.health = Math.max(0, this.health - 35);
      this.gameplaySfxAudio.playOfficeJumpscareCue(event.animatronic === 'foxy' ? 'hook-scrape' : 'bear-grab');
      this.pushStatus(event.message, 3.0);
      event = this.chapterNine.consumeJumpscareEvent();
    }
  }
  ...
  private getObjectiveText(): string {
    ...
    if (this.chapterNineActive) {
      return [
        "Chapter 9: Freddy's Pizza Complex",
        '',
        `Phase: ${this.chapterNine.getPhaseLabel()} (${Math.ceil(this.chapterNine.getPhaseRemaining())}s)`,
        `Evidence: ${this.chapterNine.getFootageCount()}/${this.chapterNine.getFootageTarget()}`,
        `Puzzles: ${this.chapterNine.getPuzzleCount()}/${this.chapterNine.getPuzzleTarget()}`,
        this.chapterNine.isEscapeUnlocked()
          ? 'Escape is unlocked. Reach the front doors and leave with the footage.'
          : 'Record rooms and moving animatronics, then solve the breaker, kitchen valve, security terminal, and front chain.',
        'Left click records. E uses vents, puzzles, and front doors.',
      ].join('\n');
    }
  ...
  private getStoryNoticeState(): { text: string; active: boolean; label: string } {
    ...
    return {
      ...
      active: this.chapterExitUnlocked && !this.chapterTwoActive && !this.officeChapterActive && !this.chapterFourActive && !this.chapterFiveActive && !this.chapterSixActive && !this.chapterSevenActive && !this.chapterEightActive && !this.chapterNineActive && !this.zombieModeActive && !this.doomModeActive,
      label: 'Chapter Shift',
    };
  }
    ...
    if (this.officeChapterActive) {
      ...
      if (employeeElevator) {
        return this.isPlayerNearOfficeEmployeeElevatorLowerButton()
          ...
          : this.officeEmployeeElevatorAtBasement
            ? 'Press E to call the employees-only elevator back up.'
  ...
  private getStatusText(): string {
    ...
    if (this.chapterNineActive) {
      if (!this.player.isLocked()) {
        return "Click the play space to continue filming Freddy's Pizza Complex.";
      }
      return this.chapterNine.isEscapeUnlocked()
        ? 'Escape is ready. Press E at the front doors to leave.'
        : 'Left click records evidence with the shoulder camera. Press E near vents, breaker panels, the security terminal, kitchen valve, or front doors.';
    }
    ...
    if (this.officeChapterActive) {
      ...
      if (employeeElevator) {
        return this.isPlayerNearOfficeEmployeeElevatorLowerButton()
          ...
          : this.officeEmployeeElevatorAtBasement
            ? 'The elevator is down in the basement. Press E to call it back up to the first floor.'
  ...
  private getSupportedFloorHeight(): number | null {
    ...
    if (this.chapterNineActive) {
      return this.chapterNine.getSupportedFloorY(this.player.getPosition());
    }
  ...
  private startOfficeEmployeeElevatorRide(
    forceDirection?: 'up' | 'down',
    carryPlayer = true,
    pressedButton: 'top' | 'bottom' = 'top',
  ): void {
    const elevator = this.officeChapter.employeeElevator;
    ...
    elevator.button.position.x = pressedButton === 'top' ? elevator.buttonRestX - 0.04 : elevator.buttonRestX;
    elevator.lowerButton.position.x = pressedButton === 'bottom' ? elevator.lowerButtonRestX - 0.04 : elevator.lowerButtonRestX;
    ...
    this.pushStatus(
      ridingUp
        ? carryPlayer
          ...
          : 'The first-floor button calls the employees-only elevator back up.'
  ...
  private updateOfficeEmployeeElevatorRide(deltaSeconds: number): void {
    ...
    elevator.button.position.x = ride.pressedButton === 'top'
      ? elevator.buttonRestX - Math.max(0, 1 - rawProgress * 5) * 0.04
      : elevator.buttonRestX;
    elevator.lowerButton.position.x = ride.pressedButton === 'bottom'
    ...
    this.officeEmployeeElevatorRide = null;
    this.officeEmployeeElevatorAtBasement = ride.endPosition.y < elevator.topPosition.y - 0.5;
    this.officeEmployeeElevatorBasementActive = ride.carryPlayer && this.officeEmployeeElevatorAtBasement;
    ...
    this.pushStatus(
      this.officeEmployeeElevatorAtBasement
        ...
        : ride.carryPlayer
          ...
          : 'The elevator arrives back at the first-floor employees-only room.',
      2.8,
    );
  }
  ...
  private handleOfficeChapterInteract(): void {
    ...
    if (this.getNearestOfficeEmployeeElevator()) {
      const pressingBasementButton = this.isPlayerNearOfficeEmployeeElevatorLowerButton();
      const callingUpFromFirstFloor = !pressingBasementButton && this.officeEmployeeElevatorAtBasement;
      this.startOfficeEmployeeElevatorRide(
        pressingBasementButton || callingUpFromFirstFloor ? 'up' : 'down',
        !callingUpFromFirstFloor,
        pressingBasementButton ? 'bottom' : 'top',
      );
      return;
    }
  ...
  private getHudJumpScareIntensity(): number {
    return Math.max(
      ...
      this.getChapterFourGreenJumpscareIntensity(),
      this.chapterNineJumpscare
        ? MathUtils.clamp(1 - this.chapterNineJumpscare.elapsed / this.chapterNineJumpscare.duration, 0, 1)
        : 0,
    );
  }
  ...
  private getHudJumpScareVariant(): HudJumpScareVariant | null {
    ...
    if (this.chapterNineJumpscare) {
      return this.chapterNineJumpscare.event.variant;
    }
