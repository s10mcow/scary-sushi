...
function createGoldenBoriPerformer(x: number, y: number, z: number): StageAnimatronicRefs {
  const performer = createStageAnimatronic('bear', x, y, z, false, false);
  performer.root.scale.set(1.34, 1.36, 1.34);
...
export function createOfficeChapter(options: OfficeChapterOptions = {}): OfficeChapterData {
  ...
  const gameRoomSideRoomNorthCenterZ = 104.96;
  ...
  const gameRoomWestSideRoomDepth = 8.4;
  ...
  const gameRoomEastSideRoomMaxX = kitchenHallRoomMaxX + gameRoomSideRoomLength;
  const gameRoomEastSideRoomCenterX = (gameRoomEastSideRoomMinX + gameRoomEastSideRoomMaxX) / 2;
  const gameRoomSideRooms = ([
    { side: 'west' as const, centerX: gameRoomWestSideRoomCenterX, minX: gameRoomWestSideRoomMinX, maxX: gameRoomWestSideRoomMaxX, wallX: kitchenHallRoomMinX, frameFacing: 1 as const },
    { side: 'east' as const, centerX: gameRoomEastSideRoomCenterX, minX: gameRoomEastSideRoomMinX, maxX: gameRoomEastSideRoomMaxX, wallX: kitchenHallRoomMaxX, frameFacing: -1 as const },
  ...
  const northPartyHallWalls: WallDefinition[] = [
    ...
    {
      position: [-221.49, WALL_HEIGHT / 2, 113.49],
      size: [5.68, WALL_HEIGHT, 0.9],
    },
    {
  ...
  const addGameRoomWallOutsideOpenings = (
    wallCenterX: number,
    openings: Array<{ openingMinZ: number; openingMaxZ: number }>,
  ): void => {
    let cursorZ = kitchenHallRoomNorthZ;
    openings
      .map((opening) => ({
        minZ: Math.max(kitchenHallRoomNorthZ, opening.openingMinZ),
        maxZ: Math.min(kitchenHallRoomSouthZ, opening.openingMaxZ),
      }))
      .filter((opening) => opening.maxZ > opening.minZ)
      .sort((a, b) => a.minZ - b.minZ)
      .forEach((opening) => {
        const depth = opening.minZ - cursorZ;
        if (depth > 0.24) {
          northPartyHallWalls.push({
            position: [wallCenterX, WALL_HEIGHT / 2, cursorZ + depth / 2],
            size: [WALL_THICKNESS, WALL_HEIGHT, depth],
          });
        }
    ...
    const finalDepth = kitchenHallRoomSouthZ - cursorZ;
    if (finalDepth > 0.24) {
      northPartyHallWalls.push({
        position: [wallCenterX, WALL_HEIGHT / 2, cursorZ + finalDepth / 2],
        size: [WALL_THICKNESS, WALL_HEIGHT, finalDepth],
      });
    }
  };
  addGameRoomWallOutsideOpenings(
    kitchenHallRoomMinX + WALL_THICKNESS / 2,
    gameRoomSideRooms.filter((room) => room.side === 'west'),
  );
  addGameRoomWallOutsideOpenings(
    kitchenHallRoomMaxX - WALL_THICKNESS / 2,
    gameRoomSideRooms.filter((room) => room.side === 'east'),
  );
  gameRoomSideRooms.forEach((room) => {
    const outerWallX = room.side === 'west'
      ? room.minX + WALL_THICKNESS / 2
      : room.maxX - WALL_THICKNESS / 2;
    northPartyHallWalls.push(
      {
      ...
      },
      {
        ...
        size: [gameRoomSideRoomLength, WALL_HEIGHT, WALL_THICKNESS],
      },
      {
        position: [room.centerX, WALL_HEIGHT / 2, room.maxZ - WALL_THICKNESS / 2],
        size: [gameRoomSideRoomLength, WALL_HEIGHT, WALL_THICKNESS],
      },
    );
  });
  ...
  const addGameRoomSideRoomDoorFrame = (room: typeof gameRoomSideRooms[number]): void => {
    const frameX = room.wallX + room.frameFacing * (WALL_THICKNESS / 2 + 0.045);
    const frameMaterial = materials.metal;
    const leftPost = new Mesh(new BoxGeometry(0.18, 2.72, 0.18), frameMaterial);
    leftPost.position.set(frameX, 1.36, room.openingMinZ);
    const rightPost = new Mesh(new BoxGeometry(0.18, 2.72, 0.18), frameMaterial);
    rightPost.position.set(frameX, 1.36, room.openingMaxZ);
    const topPost = new Mesh(new BoxGeometry(0.2, 0.18, gameRoomSideRoomDoorWidth + 0.32), frameMaterial);
    topPost.position.set(frameX, 2.78, room.centerZ);
  ...
  [
    ...
    { id: 'main-south-golden', kind: 'golden' as const, slogan: 'Stay golden stay loud', x: partyRoomCenterX - 5.7, z: partyRoomSouthZ - WALL_THICKNESS - 0.026, rotationY: Math.PI },
    ...
    { id: 'game-west-room-golden', kind: 'golden' as const, slogan: 'Gold room gold rules', x: gameRoomWestSideRoomMinX + WALL_THICKNESS + 0.026, z: gameRoomSideRoomSouthCenterZ, rotationY: Math.PI / 2 },
    { id: 'game-east-room-golden', kind: 'golden' as const, slogan: 'Bori sees the games', x: gameRoomEastSideRoomMaxX - WALL_THICKNESS - 0.026, z: gameRoomSideRoomNorthCenterZ, rotationY: -Math.PI / 2 },
    { id: 'game-east-golden-wall', kind: 'golden' as const, slogan: 'Play games win gold', x: kitchenHallRoomMaxX - WALL_THICKNESS - 0.026, z: kitchenHallRoomCenterZ + 6.2, rotationY: -Math.PI / 2 },
    { id: 'game-west-golden-wall', kind: 'golden' as const, slogan: 'Golden laugh party', x: kitchenHallRoomMinX + WALL_THICKNESS + 0.026, z: kitchenHallRoomCenterZ + 4.2, rotationY: Math.PI / 2 },
    ...
    { id: 'north-hall-east-golden', kind: 'golden' as const, slogan: 'Golden Bori listens', x: northPartyHallOpeningMaxX - WALL_THICKNESS - 0.026, z: northPartyHallCenterZ - 2.2, rotationY: -Math.PI / 2 },
    { id: 'north-hall-west-golden', kind: 'golden' as const, slogan: 'Gold waits ahead', x: northPartyHallOpeningMinX + WALL_THICKNESS + 0.026, z: northPartyHallCenterZ + 3.4, rotationY: Math.PI / 2 },
    ...
    { id: 'pirate-hall-golden', kind: 'golden' as const, slogan: 'Gold follows the show', x: secondHallCenterX + 3.1, z: secondHallOpeningMinZ + WALL_THICKNESS + 0.026, rotationY: 0 },
    ...
    { id: 'pirate-west-golden', kind: 'golden' as const, slogan: 'Golden treasure smiles', x: secondRoomMinX + WALL_THICKNESS + 0.03, z: secondRoomCenterZ + 1.4, rotationY: Math.PI / 2 },
    ...
    { id: 'ballpit-west-golden', kind: 'golden' as const, slogan: 'Gold dives deeper', x: northPartySideRoomMinX + WALL_THICKNESS + 0.026, z: northPartySideRoomCenterZ - 3.8, rotationY: Math.PI / 2 },
    { id: 'ballpit-south-golden', kind: 'golden' as const, slogan: 'Golden prizes below', x: northPartySideRoomCenterX + 3.5, z: northPartySideRoomMaxZ - WALL_THICKNESS - 0.026, rotationY: Math.PI },
    ...
    { id: 'backstage-hall-golden', kind: 'golden' as const, slogan: 'Golden staff only', x: backstageHallMinX + WALL_THICKNESS + 0.026, z: backstageHallCenterZ + 1.2, rotationY: Math.PI / 2 },
    { id: 'backstage-storage-east-golden', kind: 'golden' as const, slogan: 'Gold behind the curtain', x: backstageStorageMaxX - WALL_THICKNESS - 0.026, z: backstageStorageCenterZ + 1.8, rotationY: -Math.PI / 2 },
    ...
    { id: 'storage-closet-golden', kind: 'golden' as const, slogan: 'Gold hides in storage', x: storageClosetCenterX - 1.15, z: storageClosetMaxZ - WALL_THICKNESS - 0.026, rotationY: Math.PI },
    ...
    { id: 'kitchen-east-golden', kind: 'golden' as const, slogan: 'Golden pizza party', x: kitchenEastX - WALL_THICKNESS - 0.026, z: kitchenCenterZ + 2.2, rotationY: -Math.PI / 2 },
    { id: 'kitchen-north-golden', kind: 'golden' as const, slogan: 'Gold cooks after dark', x: kitchenCenterX + 2.8, z: kitchenMinZ + WALL_THICKNESS + 0.026, rotationY: 0 },
    ...
    { id: 'bathroom-hall-golden', kind: 'golden' as const, slogan: 'Golden clean hands', x: bathroomHallCenterX + 0.95, z: bathroomEntryMaxZ - WALL_THICKNESS - 0.026, rotationY: Math.PI },
  ].forEach((poster) => {
  ...
  basementSideRooms.forEach((room) => {
    ...
    basementHallwayBounds.push({
      ...
      minZ: room.openingMinZ + 0.12,
      maxZ: room.openingMaxZ - 0.12,
    });
  });
  ...
  const addBasementFlickerFixture = (
    ...
    const bulbMaterial = new MeshStandardMaterial({
      ...
      emissive: 0xffd8a4,
      emissiveIntensity: condition === 'broken' ? 0.28 : condition === 'dim' ? 0.68 : 1.4,
      roughness: 0.28,
      metalness: 0.02,
    });
    const bulb = new Mesh(
    ...
    const conditionPower = condition === 'broken' ? 0.18 : condition === 'dim' ? 0.5 : 1;
    basementFlickerLights.push({
      light,
      material,
      bulbMaterial,
      baseIntensity: baseIntensity * 1.52 * conditionPower,
      phase,
      fault: 0.45 + (phase % 1.9),
      damage: condition === 'broken' ? 1 : condition === 'dim' ? 0.42 : 0,
      minimumPower: condition === 'broken' ? 0.04 : condition === 'dim' ? 0.3 : 0.72,
    });
  ...
  addBasementFlickerFixture(southBasementHallwayCenterX, blockedBasementRoomCenterZ, 1.85, 4.85, 21, 'broken');
  ...
  const shaftGlowTop = new PointLight(0xffddb0, 0.95, 7.2, 1.58);
  shaftGlowTop.position.set(employeeElevatorCenterX, -1.45, employeeElevatorCenterZ);
  const shaftGlowLower = new PointLight(0xffddb0, 1.08, 8.8, 1.62);
  ...
  const basementGlow = new PointLight(0xffdfb5, 2.7, 17.5, 1.48);
  basementGlow.position.set(employeeElevatorCenterX + 1.8, employeeElevatorBasementFloorY + 2.22, employeeElevatorCenterZ - 1.9);
  const basementStaticLights = [
    { light: shaftGlowTop, baseIntensity: 0.95 },
    { light: shaftGlowLower, baseIntensity: 1.08 },
    { light: basementGlow, baseIntensity: 2.7 },
  ];
  ...
  const updateGoldenBoriWander = (
    ...
    const armStrength = goldenBoriChaseActive ? 0.38 : 0.26;
    goldenBori.leftArm.root.rotation.x = -0.08 + leftSwing * armStrength;
    goldenBori.leftArm.root.rotation.z = -0.16 + Math.max(0, -leftSwing) * 0.1;
    goldenBori.leftArm.joint.rotation.x = 0.08 + Math.max(0, leftSwing) * 0.22;
    goldenBori.rightArm.root.rotation.x = -0.08 + rightSwing * armStrength;
    goldenBori.rightArm.root.rotation.z = 0.16 - Math.max(0, -rightSwing) * 0.1;
    goldenBori.rightArm.joint.rotation.x = 0.08 + Math.max(0, rightSwing) * 0.22;
    if (playerPosition && goldenBoriChaseActive) {
      goldenBori.root.updateMatrixWorld(true);
      const localTarget = goldenBori.root.worldToLocal(partyLocalTarget.copy(playerPosition));
      const direction = localTarget.sub(goldenBori.head.position);
      goldenBori.head.rotation.y = MathUtils.clamp(Math.atan2(-direction.x, -direction.z), -0.5, 0.5);
      goldenBori.head.rotation.x = -MathUtils.clamp(
        Math.atan2(direction.y, Math.hypot(direction.x, direction.z)),
        -0.24,
        0.28,
  ...
  const update = (
    ...
    const basementVisibilityTarget = playerPosition && playerPosition.y < -1.2 ? 1 : 0.035;
    const basementVisibilityBlend = 1 - Math.exp(-deltaSeconds * 7.5);
    basementLightVisibility += (basementVisibilityTarget - basementLightVisibility) * basementVisibilityBlend;
