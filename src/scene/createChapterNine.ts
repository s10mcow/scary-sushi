...
export function createChapterNine(): ChapterNineData {
  ...
  const addBoxingMachine = (x: number, z: number, rotationY = 0): void => {
    ...
    const base = new Mesh(new BoxGeometry(1.72, 1.08, 1.26), cabinet);
    base.position.y = 0.54;
    const frontPanel = new Mesh(new BoxGeometry(1.42, 0.44, 0.06), new MeshStandardMaterial({ color: 0x5c2026, roughness: 0.6, metalness: 0.06 }));
    frontPanel.position.set(0, 0.64, 0.66);
    const footPlate = new Mesh(new BoxGeometry(1.9, 0.08, 1.42), blackMetalMaterial);
    footPlate.position.y = 0.04;
    const post = new Mesh(new CylinderGeometry(0.11, 0.15, 2.28, 18), blackMetalMaterial);
    post.position.set(0, 1.88, -0.08);
    const topHousing = new Mesh(new BoxGeometry(1.12, 0.46, 0.52), cabinet);
    topHousing.position.set(0, 2.86, 0.06);
    const hingeArm = new Mesh(new CylinderGeometry(0.04, 0.04, 0.5, 10), metalMaterial);
    hingeArm.position.set(0, 2.72, 0.28);
    hingeArm.rotation.x = Math.PI / 2;
    ...
    [-0.46, 0.46].forEach((lightX) => {
      const light = new Mesh(new SphereGeometry(0.08, 10, 8), new MeshBasicMaterial({ color: lightX < 0 ? 0xff3344 : 0xffd84d }));
      light.position.set(lightX, 1.24, 0.69);
      game.add(light);
    });
  ...
  const addDancePadGame = (x: number, z: number, rotationY = 0): void => {
    ...
    game.position.set(x, 0, z);
    game.rotation.y = rotationY;
    const standMaterial = new MeshStandardMaterial({ color: 0x181e2d, roughness: 0.55, metalness: 0.08 });
    const platform = new Mesh(new BoxGeometry(2.28, 0.22, 1.82), new MeshStandardMaterial({ color: 0x262b36, roughness: 0.62, metalness: 0.12 }));
    platform.position.y = 0.11;
    const platformTrim = new Mesh(new BoxGeometry(2.42, 0.08, 1.96), blackMetalMaterial);
    platformTrim.position.y = 0.24;
    const tvStand = new Mesh(new BoxGeometry(1.88, 1.28, 0.32), standMaterial);
    tvStand.position.set(0, 0.98, -0.84);
    const tv = new Mesh(new BoxGeometry(2.08, 1.18, 0.18), blackMetalMaterial);
    tv.position.set(0, 1.9, -0.78);
    const screenFace = new Mesh(new PlaneGeometry(1.72, 0.86), createSignMaterial('STEP', 'BEAT', '#f062c0'));
    screenFace.position.set(0, 1.9, -0.68);
    const smallSign = new Mesh(new PlaneGeometry(1.18, 0.28), createSignMaterial('DANCE', '', '#f7d8ff'));
    smallSign.position.set(0, 2.58, -0.68);
    const speakerLeft = new Mesh(new CylinderGeometry(0.12, 0.16, 0.06, 18), metalMaterial);
    speakerLeft.position.set(-0.66, 1.1, -0.66);
    speakerLeft.rotation.x = Math.PI / 2;
    const speakerRight = speakerLeft.clone();
    speakerRight.position.x = 0.66;
    [-0.42, 0.42].forEach((padX) => {
      [-0.1, 0.62].forEach((padZ, index) => {
        const pad = new Mesh(new BoxGeometry(0.62, 0.08, 0.62), new MeshStandardMaterial({ color: index === 0 ? 0x35a6d8 : 0xd8357c, roughness: 0.45, metalness: 0.12 }));
        pad.position.set(padX, 0.3, padZ);
        game.add(pad);
      });
    });
    game.add(platform, platformTrim, tvStand, tv, screenFace, smallSign, speakerLeft, speakerRight);
    root.add(game);
  ...
  const addPirateCoveDarkZone = (): void => {
    ...
    const darkFloor = new Mesh(new PlaneGeometry(45, 96), darkMaterial);
    darkFloor.position.set(-43.8, 0.045, -17.6);
    darkFloor.rotation.x = -Math.PI / 2;
    root.add(darkFloor);

    const darkCeiling = new Mesh(new PlaneGeometry(45, 96), new MeshBasicMaterial({
      color: 0x02020a,
      transparent: true,
      opacity: 0.42,
      side: DoubleSide,
    ...
    darkCeiling.position.set(-43.8, WALL_HEIGHT - 0.18, -17.6);
    darkCeiling.rotation.x = Math.PI / 2;
    root.add(darkCeiling);

    const sideWallTint = new Mesh(new PlaneGeometry(96, WALL_HEIGHT), new MeshBasicMaterial({
      color: 0x03020a,
      transparent: true,
      opacity: 0.18,
      side: DoubleSide,
    ...
    sideWallTint.position.set(-64.62, WALL_HEIGHT / 2, -17.6);
    sideWallTint.rotation.y = Math.PI / 2;
    root.add(sideWallTint);
  };
  ...
  const addDiscoBall = (x: number, z: number, lightColors: number[]): void => {
    ...
    for (let band = -1; band <= 1; band += 1) {
      for (let column = 0; column < 6; column += 1) {
        ...
        const angle = (column / 6) * Math.PI * 2 + band * 0.16;
    ...
    lightColors.forEach((color, index) => {
      ...
      const makeBeamMaterial = (opacity: number): MeshBasicMaterial => new MeshBasicMaterial({
        color,
        transparent: true,
        opacity,
        side: DoubleSide,
      ...
      const addBeam = (end: Vector3, radius: number, opacity: number): void => {
        const beamDirection = beamStart.clone().sub(end);
        const beamLength = beamDirection.length();
        const beam = new Mesh(new ConeGeometry(radius, beamLength, 10, 1, true), makeBeamMaterial(opacity));
        beam.position.copy(beamStart).add(end).multiplyScalar(0.5);
        beam.quaternion.setFromUnitVectors(new Vector3(0, 1, 0), beamDirection.normalize());
        fixture.add(beam);
      };
      const downEnd = new Vector3(Math.cos(angle) * 4.2, 0.1, Math.sin(angle) * 4.2);
      const sideEnd = new Vector3(Math.cos(angle + 0.32) * 8.4, 2.6 + (index % 2) * 0.5, Math.sin(angle + 0.32) * 8.4);
      const highEnd = new Vector3(Math.cos(angle - 0.48) * 6.2, WALL_HEIGHT - 0.32, Math.sin(angle - 0.48) * 6.2);
      addBeam(downEnd, 0.9, 0.24);
      addBeam(sideEnd, 0.62, 0.16);
      if (index % 2 === 0) {
        addBeam(highEnd, 0.46, 0.12);
      }

      const floorSpot = new Mesh(new PlaneGeometry(2.45, 0.42), spotMaterial);
      floorSpot.position.copy(downEnd);
      floorSpot.position.y = 0.11;
      floorSpot.rotation.x = -Math.PI / 2;
      floorSpot.rotation.z = angle + Math.PI / 2;
      fixture.add(floorSpot);

      const secondFloorSpot = new Mesh(new PlaneGeometry(1.95, 0.28), spotMaterial.clone());
      secondFloorSpot.position.set(Math.cos(angle + 0.55) * 5.25, 0.115, Math.sin(angle + 0.55) * 5.25);
      secondFloorSpot.rotation.x = -Math.PI / 2;
      secondFloorSpot.rotation.z = angle + 0.85;
      fixture.add(secondFloorSpot);

      const sideSpot = new Mesh(new PlaneGeometry(2.2, 0.32), spotMaterial.clone());
      sideSpot.position.copy(sideEnd);
      sideSpot.rotation.y = angle + Math.PI / 2;
      sideSpot.rotation.z = 0.12;
      fixture.add(sideSpot);
    });
  ...
  addPirateCoveDarkZone();
  addDiscoBall(-58.5, 24.5, [0xff3f8f, 0x42d5ff, 0xffdb4d]);
  addDiscoBall(-58.5, 10, [0x7a5cff, 0x50ff9c, 0xff5a38]);
  addDiscoBall(-57, -4, [0x64ffda, 0xff4a65, 0xf5ff5d]);
  addDiscoBall(-54, -55, [0x2ecbff, 0xff3f8f, 0x79ff62]);
  addDiscoBall(-39, -55, [0xff5a38, 0x7a5cff, 0x42d5ff]);
  addDiscoBall(-35, -41.5, [0x64ffda, 0xff4ec6, 0xffdb4d]);
