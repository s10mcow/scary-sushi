...
function addCounterSinkOvenAndWasher(root: Group, colliders: CollisionBox[]): ChapterTenSink {
  ...
  const verticalPipe = new Mesh(new CylinderGeometry(0.045, 0.048, 0.46, 16), metalMaterial);
  verticalPipe.name = 'Chapter 10 sink vertical faucet pipe';
  verticalPipe.position.set(0.04, 1.65, -0.48);
  verticalPipe.castShadow = true;
  sinkCounter.add(verticalPipe);
  const elbow = new Mesh(new TorusGeometry(0.16, 0.038, 10, 18, Math.PI / 2), metalMaterial);
  elbow.name = 'Chapter 10 sink left curving faucet elbow';
  elbow.position.set(0.04, 1.88, -0.32);
  elbow.rotation.x = Math.PI / 2;
  elbow.rotation.z = Math.PI;
  elbow.castShadow = true;
  sinkCounter.add(elbow);
  const sidePipe = new Mesh(new CylinderGeometry(0.043, 0.046, 0.36, 14), metalMaterial);
  sidePipe.name = 'Chapter 10 sink pipe curving left toward bowl';
  sidePipe.position.set(0.04, 1.88, -0.02);
  sidePipe.rotation.x = Math.PI / 2;
  sidePipe.castShadow = true;
  sinkCounter.add(sidePipe);
  const downSpout = new Mesh(new CylinderGeometry(0.04, 0.045, 0.34, 14), metalMaterial);
  downSpout.name = 'Chapter 10 sink downward faucet spout';
  downSpout.position.set(0.04, 1.71, 0.16);
  downSpout.castShadow = true;
  sinkCounter.add(downSpout);
  const spoutTip = new Mesh(new CylinderGeometry(0.052, 0.048, 0.07, 14), metalMaterial);
  spoutTip.name = 'Chapter 10 sink faucet nozzle tip';
  spoutTip.position.set(0.04, 1.51, 0.16);
  spoutTip.castShadow = true;
  sinkCounter.add(spoutTip);
  const handlePivot = new Group();
  handlePivot.name = 'Chapter 10 sink twist handle pivot';
  handlePivot.position.set(0.34, 1.5, -0.45);
  const handleStem = new Mesh(new CylinderGeometry(0.035, 0.035, 0.16, 12), metalMaterial);
  handleStem.name = 'Chapter 10 sink twist handle short stem';
  handleStem.rotation.z = Math.PI / 2;
  const handleBar = new Mesh(new BoxGeometry(0.42, 0.055, 0.09), metalMaterial);
  handleBar.name = 'Chapter 10 sink left right twist handle';
  handleBar.position.x = 0.12;
  ...
  waterStream.position.set(0.04, 1.17, 0.16);
